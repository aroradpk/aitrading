...
- **Pattern scoring:** `app/engines/pattern_scoring.py` — ladder, not a yes/no 5% test. **5** = daily coil, expect ~**3%**. **6** = daily rumble (range ≥ 2.5%, close not ±5%), expect ~**4%**. **7** = rumble **plus both** 15m and 1h precision (15m wedge or coil@EMA20, **and** 1h coil@EMA20 or rounding@EMA20). Goal is **fewer, cleaner 7s**, not catching every 5% day. Hourly Fib is a cherry only (fires almost every day — never a 7-gate). The 5% close itself is a **late bar** and must not be a 7. Intraday cache: `python scripts/fetch_intraday.py` when `offline_mode: false` (Yahoo 15m/1h is ~60 days). Without 15m/1h files, rumble stays a **6**. Eval: `python scripts/eval_same_day_conviction.py` (rumble day-before).
