...
- **Pattern scoring:** `app/engines/pattern_scoring.py` — a **7** is a **same-day wide bar** (range ≥ 1.3× ATR). That hits ~80% of |close|≥5% days at ≤20% quiet FPR on Next 50 + Nifty 50 top 20. Coil-only is capped at 5. Predicting the *next* day’s 5% move at 80% recall still needs ~40% FPR and is not the 7. Extra names: `config/nifty_50_top20.json`. Search: `python scripts/search_fpr_recall.py`. Compare: `python scripts/compare_weighted_vs_energy.py`.
